...
begin
  ...
  AssertTrue('Form data should exist in response', FormData.IndexOfName('test') >= 0);
...
begin
  ...
  AssertTrue('Custom header should be echoed back',
    Headers.Get('X-Custom-Header', '') = 'test');
  Headers.Free;
end;
...
begin
  ...
  AssertNotNull('Args should exist in response', Args);
...
begin
  ...
  AssertTrue('Authorization header should exist',
    Headers.IndexOfName('Authorization') >= 0);
  Headers.Free;
end;
...
begin
  ...
  AssertNotNull('JSON data should exist in response', JsonData);
...
begin
  ...
  FormData := TJSONObject(Response.JSON.FindPath('form'));
  AssertNotNull('Form data should exist in response', FormData);
